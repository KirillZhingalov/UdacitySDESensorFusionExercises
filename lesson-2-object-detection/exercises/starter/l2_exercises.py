...
def plot_precision_recall(det_performance_all): 
    ...
    precision, recall = [], []
    for conf_th in np.linspace(0.1, 0.9, 9):
        prec, rec = compute_precision_recall(det_performance_all, conf_th)
...
def compute_precision_recall(det_performance_all, conf_thresh=0.5):
    # TODO fix: confidence th is not used while computing Precision/Recall
    
    if not len(det_performance_all):
    ...
    positive_negative_array = np.array([el[2] for el in det_performance_all])
    
    tp = positive_negative_array[:,1].sum()
    fn = positive_negative_array[:,2].sum()
    fp = positive_negative_array[:,3].sum()
...
def pcl_to_bev(lidar_pcl, configs, vis=True):
    ...
    if vis:
        print('Height map min/max:', height_map.min(), height_map.max())
